...
mod tests {
    ...
    /// Hits the real API when DICTEA_MISTRAL_KEY is set, skipped otherwise.
    ...
    async fn transcribes_through_the_live_api() {
        let Ok(key) = std::env::var("DICTEA_MISTRAL_KEY") else {
            eprintln!("DICTEA_MISTRAL_KEY not set, skipping");
            return;
        };
        ...
        let text = VoxtralEngine::transcribe_async(reqwest::Client::new(), key, samples, None)
            .await
